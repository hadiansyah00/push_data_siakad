...
class JadwaluapModel extends CI_Model
{
...
public function delete_data_uap($id_jadwal) {
	  ...
	  public function getDataPraSW()
	{
	 ...
	 public function getDataPra($id)
	{
		$this->db->select('*');
		$this->db->from('jadwal_pra_uap');
		$this->db->join('ta', 'ta.id_ta = jadwal_pra_uap.id_ta');
		$this->db->join('jurusan', 'jurusan.kd_jurusan = jadwal_pra_uap.kd_jurusan');
		$this->db->where('jadwal_pra_uap.kd_jurusan', $id);
		$this->db->where('status', 1);
		$query = $this->db->get();
		return $query;
	}
