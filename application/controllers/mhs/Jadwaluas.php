...
class Jadwaluas extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['getUas_karyawan'] = $this->JadwaluasModel->getMatkul_karyawan($mhs['kd_jurusan']);
	...
	public function print_uas_kelas_pagi($data)
	{
		...
		$getUas = $this->JadwaluasModel->getMatkul_KRS($mhs['kd_jurusan']);
		...
		foreach ($getUas as $d) {
			...
			$pdf->SetFont('Times', '', 9);
            ob_end_clean();
		    $pdf->Output( 'Kartu UAS STIKes Bogor Husada Reguler A'.'_'. $mhs['nama_mhs'].'.pdf','D');
	}
	
		public function print_uas_kelas_karyawan($data)
	{
		$data['title'] = 'Kartu UAS';
		$data['judul'] = 'Kartu UaS Mahasiswa';
        $mhs = $this->KrsModel->getDataMhs();
		...
		//setting krs
		$setting = $this->db->get('set_krs')->row_array();
        $getUas_karyawan= $this->JadwaluasModel->getMatkul_karyawan($mhs['kd_jurusan']);
		$tahun = $this->TaModel->getAktif()->row_array();
		//get data from session
		$mhs = $this->KrsModel->getDataMhs();
		//get Jadwal for mahasiswa
		...
		$pdf->Output( 'Kartu UAS STIKes Bogor Husada Reguler B'.'_'. $mhs['nama_mhs'].'.pdf','D');
	}
}