...
class Jadwal_uap extends CI_Controller
{
...
// 		$data['dataUap'] = $this->JadwaluapModel->getDataUap($id)->result();
	...
	public function insert($kd_jurusan)
	{
		...
		$data = array(
			...
			'jam_uap'			=> htmlspecialchars($this->input->post('jam_uap')),
			'tanggal_uap'	    => htmlspecialchars($this->input->post('tanggal_uap')),
		);

		//var_dump($data);
		$this->JadwaluapModel->insertData('jadwal_uap', $data);
...
// 			'hari_uas'			=> htmlspecialchars($this->input->post('hari_uas')),
	...
	public function delete($id)
	{
		...
		$this->db->delete('jadwal_uap', $where);
		$this->session->set_flashdata(
			'pesan',
